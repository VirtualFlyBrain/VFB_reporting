...
ref_terms = ['UPDATED', 'LINKED', 'Paper', 'et al.', ' from ',
             '?', 'on server', 'assigned to', 'update ', 'need to ']
...
def find_offical_label(term):
    ...
    if not ' soma' in term and ('left ' in term or 'right ' in term):
        ...
        expanded.append('adult ' + term.replace('left ',
                        '').replace('right ', ''))
        expanded.append(modified.replace('left ', '').replace('right ', ''))
        expanded.append('adult ' + modified.replace('left ',
                        '').replace('right ', ''))
        ...
        for annotation in annotations.split('), '):
            ...
            if 'project id 2 on server https://catmaid3.hms.harvard.edu/catmaidvnc' in name:
                if 'LINKED NEURON - elastic transformation and flipped of skeleton id ' in name:
                    result += "|" + name.replace('LINKED NEURON - elastic transformation and flipped of skeleton id ', 'catmaid_fanc:').replace(
                        ' in project id 2 on server https://catmaid3.hms.harvard.edu/catmaidvnc', '')
                else:
                    result += "|" + name.replace('LINKED NEURON - elastic transformation of skeleton id ', 'catmaid_fanc:').replace(
                        ' in project id 2 on server https://catmaid3.hms.harvard.edu/catmaidvnc', '')
            if 'source: https://gen1mcfo.janelia.org/cgi-bin/view_gen1mcfo_imagery.cgi?line=' in name:
                result += "|" + \
                    name.replace(
                        'source: https://gen1mcfo.janelia.org/cgi-bin/view_gen1mcfo_imagery.cgi?line=', 'FlyLightGen1MCFO:')
            if 'project id 59 on server https://catmaid3.hms.harvard.edu/catmaidvnc' in name:
                if 'LINKED NEURON - pruned (first entry, last exit) by vol 109 of skeleton id ' in name:
                    result += "|" + name.replace('LINKED NEURON - pruned (first entry, last exit) by vol 109 of skeleton id ',
                                                 'catmaid_fanc_JRC2018VF:').replace(' in project id 59 on server https://catmaid3.hms.harvard.edu/catmaidvnc', '')
                if 'LINKED NEURON - radius pruned of skeleton id ' in name:
                    result += "|" + name.replace('LINKED NEURON - radius pruned of skeleton id ', 'catmaid_fanc_JRC2018VF:').replace(
                        ' in project id 59 on server https://catmaid3.hms.harvard.edu/catmaidvnc', '')
        xrefs.append(result)
...
def create_metadata(db="", filename_series=[], annotation_series=[], pub=""):
    results = []
    soma_rules = {'left soma': 'left side of organism', 'right soma': 'right side of organism',
                  'midline soma': 'cell body rind along midline of adult ventral nerve cord'}
    nerve_rules = {}
    for id, annotations in zip(filename_series, annotation_series):
        if ' soma' in annotations:
            for key in soma_rules.keys():
                if key in annotations:
                    results.append({'object': soma_rules[key], 'relation': 'has soma location',
                                   'subject_external_id': id, 'subject_external_db': db, 'pub': pub})
        else:
            if 'left ' in annotations:
                results.append({'object': 'left side of organism', 'relation': 'overlaps',
                               'subject_external_id': id, 'subject_external_db': db, 'pub': pub})
        ...
        if 'nerve' in annotations or 'bundle' in annotations:
            for annotation in annotations.split('), '):
                ...
                if len(name) > 0 and ('nerve' in name or 'nerve' in annotation):
                    results.append({'object': name, 'relation': 'fasciculates_with',
                                   'subject_external_id': id, 'subject_external_db': db, 'pub': pub})
        if 'motor neuron' in annotations or 'sensory neuron' in annotations or 'nerve' in annotations:
            results.append({'object': 'peripheral nervous system', 'relation': 'overlaps',
                           'subject_external_id': id, 'subject_external_db': db, 'pub': pub})
...
def make_anat_records(site, curator, output_filename='./anat'):
    ...
    new_skids = pd.read_csv("%s%s_new_skids.tsv" % (save_directory, site), sep='\t', dtype='str')
    ...
    for i in paper_ids:
        ...
        if site == 'FAFB':
            ...
            curation_df = pd.DataFrame({'filename': single_ds_data['skid'],
                                    ...
                                    'is_a': find_available_terms(single_ds_data['name'].map(
                                        lambda x: str('%s' % ("".join(itertools.dropwhile(str.isdigit, x)).strip())))),
                                    'part_of': resolve_entity(entity, single_ds_data['annotations']),
                                    'comment': generate_comments(single_ds_data['annotations'])})
        else:
            curation_df = pd.DataFrame({'filename': single_ds_data['skid'],
        ...
        if 'synonyms' in single_ds_data.keys():
            curation_df['synonyms'] = single_ds_data['synonyms'].fillna('', inplace=True)
...
#         curation_df['label'] = curation_df[['label', 'filename']].apply(lambda x: ' '.join(x), axis=1)
        ...
        paper_metadata = create_metadata(db="catmaid_" + site.lower().replace('fanc2', 'fanc_JRC2018VF').replace('fanc1','fanc'), filename_series=single_ds_data['skid'], annotation_series=single_ds_data['annotations'])
        if len(paper_metadata) > 0:
            paper_metadata.to_csv(
...
if __name__ == "__main__":
    ...
    pd.DataFrame({'missing terms': pd.Series(missing.keys()), 'meta': pd.Series(missing.values())}).to_csv(
        '../VFB_reporting_results/CATMAID_SKID_reports/anat_fanc1_missing_terms.tsv', sep='\t', index=None)
    pd.DataFrame({'CATMAID annotation': pd.Series(passed.keys()), 'FBbt term': pd.Series(passed.values())}).to_csv(
        '../VFB_reporting_results/CATMAID_SKID_reports/anat_fanc1_resolved_terms.tsv', sep='\t', index=None)
    missing.clear()
    make_anat_records('FANC2', 'travis',
                      '../VFB_reporting_results/anat_fanc2_missing')
    pd.DataFrame({'missing terms': pd.Series(missing.keys()), 'meta': pd.Series(missing.values())}).to_csv(
        '../VFB_reporting_results/CATMAID_SKID_reports/anat_fanc2_missing_terms.tsv', sep='\t', index=None)
    pd.DataFrame({'CATMAID annotation': pd.Series(passed.keys()), 'FBbt term': pd.Series(passed.values())}).to_csv(
        '../VFB_reporting_results/CATMAID_SKID_reports/anat_fanc2_resolved_terms.tsv', sep='\t', index=None)
    make_anat_records('LEG40', 'travis',
                      '../VFB_reporting_results/anat_leg40_missing')
    pd.DataFrame({'missing terms': pd.Series(missing.keys()), 'meta': pd.Series(missing.values())}).to_csv(
        '../VFB_reporting_results/CATMAID_SKID_reports/anat_leg40_missing_terms.tsv', sep='\t', index=None)
    pd.DataFrame({'CATMAID annotation': pd.Series(passed.keys()), 'FBbt term': pd.Series(passed.values())}).to_csv(
        '../VFB_reporting_results/CATMAID_SKID_reports/anat_leg40_resolved_terms.tsv', sep='\t', index=None)
